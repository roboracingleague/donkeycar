...
@lazy_calibration
def calibrate_camera(calib_images_dir, plot=False):
    """
...
def undistort(frame, mtx, dist, plot=False):
    """
...
if __name__ == '__main__':
    ...
    img = cv2.imread('test_images/6_cam_image_array_.jpg')
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

    img_undistorted = undistort(img, mtx, dist, plot=True)

    cv2.imwrite('img/test_calibration_before.jpg', cv2.cvtColor(img, cv2.COLOR_RGB2BGR))
    cv2.imwrite('img/test_calibration_after.jpg', cv2.cvtColor(img_undistorted, cv2.COLOR_RGB2BGR))