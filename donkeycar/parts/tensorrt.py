...
class TensorRTLinear(KerasPilot):
    ...
    def interpreter_to_output(self, interpreter_out):
        if len(interpreter_out) == 3:
            [scene, throttle, steering] = interpreter_out
            #print(f"steering={steering[0]} throttle={throttle[0]}")
            # return -0.25, 0.3
            # return steering[0], throttle[0]
            return steering[0], throttle[0], scene[0]
...
class TensorRTSceneDetector(KerasPilot):
    '''
    Uses TensorRT to do the inference.
    '''    
    def __init__(self, cfg):
        ...
        self.stream = None
        self.context = None
        print(f'inside TensorRTSceneDetector')

    def compile(self):
        print('Nothing to compile')

    def create_model(self):
        print("should not create TensorRTSceneDetector model, it is created using post processing")
