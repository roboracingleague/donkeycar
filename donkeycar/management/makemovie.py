...
class MakeMovie(object):
    ...
    @staticmethod
    def draw_line_into_image(angle, throttle, is_left, img, color, revert_angle=True, fix_throttle=None):
        import cv2
        ...
        if revert_angle:
            angle=-angle
        a1 = angle * 45.0
        l1 = throttle * length if not fix_throttle else length*fix_throttle
